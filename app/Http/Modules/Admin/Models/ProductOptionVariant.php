...
class ProductOptionVariant extends Model
{
     ...
     * @author Akash M. Pai
     */
    public function getOptionVariantWhere($where, $selectedColumns = ['*'])
    {
        $returnData = array('code' => 400, 'message' => 'Argument Not Passed.', 'data' => null);
        if (func_num_args() > 0) {
            $where = func_get_arg(0);
            $result = DB::table('product_options')
                ->whereRaw($where['rawQuery'], isset($where['bindParams']) ? $where['bindParams'] : array())
                ->leftjoin('product_option_variants as ov', function ($join) {
                    $join->on('ov.option_id', '=', 'product_options.option_id');
                })
                ->get();
            if ($result) {
                $returnData['code'] = 200;
     ...
     */
    /*
     * STILL TESTING NOT USED YET
     */
    public function getOptionsWithVarsWhere($where, $selectedColumns = ['*'])//, $whereForJoin = ''
    {
        $returnData = array('code' => 400, 'message' => 'Argument Not Passed.', 'data' => null);
        if (func_num_args() > 0) {
            $where = func_get_arg(0);
//            $whereForJoin = func_get_arg(2);//todo
...
//                ->leftjoin('product_options as po', function ($join) {
//                    $join->on('po.option_id', '=', 'product_option_variants.option_id');
////                    $join->whereRaw($whereForJoin['rawQuery'], isset($whereForJoin['bindParams']) ? $whereForJoin['bindParams'] : array());
//                })
//                ->select(DB::raw("product_option_variants.*, po.*, CONCAT('[', GROUP_CONCAT(DISTINCT CONCAT('{\"VID\":\"',pov.variant_id,'\",\"VN\":\"',pov.variant_name,'\",\"PM\":\"',pov.price_modifier,'\",\"PMT\":\"',pov.price_modifier_type,'\",\"WM\":\"',pov.weight_modifier,'\",\"WMT\":\"',pov.weight_modifier_type,'\",\"STTS\":\"',pov.status,'\"}') ORDER BY pov.variant_id SEPARATOR ','),']') AS all_variant_data"))
//                ->distinct('product_option_variants.option_id')
//                ->distinct('pov.variant_id')
//                ->groupBy('product_option_variants.option_id')->toSql());
            $result = DB::table($this->table)
                ->whereRaw($where['rawQuery'], isset($where['bindParams']) ? $where['bindParams'] : array())
                ->leftjoin('product_option_variants as pov', function ($join) {
                    $join->on('pov.option_id', '=', 'product_option_variants.option_id');
...
//                    $join->whereRaw($whereForJoin['rawQuery'], isset($whereForJoin['bindParams']) ? $whereForJoin['bindParams'] : array());
            ...
            if ($result) {
                $returnData['code'] = 200;
                $returnData['message'] = 'All options with variants.';
            } else {
                $returnData['code'] = 400;
                $returnData['message'] = 'No data found.';
            }
     ...
     * @author Akash M. Pai <akashpai@globussoft.in>
     */
    public function addVariantAndGetID($data)
    {
        if (func_num_args() > 0) {
            $data = func_get_arg(0);
            try {
                $result = DB::table($this->table)->insertGetId($data, 'variant_id');
                return json_encode(array('code' => 200, 'message' => 'Option variant added successfully.', 'data' => $result));
            } catch (\Exception $e) {
