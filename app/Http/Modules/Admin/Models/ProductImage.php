...
class ProductImage extends Model
{
    ...
    public function getImageWhere($where, $selectedColumns = ['*'], $sortBy = ['column' => 'product_images.pi_id', 'order' => 'asc'])
    {
        $returnData = array('code' => 400, 'message' => 'Argument Not Passed.', 'data' => null);
        if (func_num_args() > 0) {
            $where = func_get_arg(0);
            $result = DB::table($this->table)
                ...
                ->join('products', function ($join) {
                    $join->on('products.product_id', '=', 'product_images.for_product_id');
                })
                ->select($selectedColumns)
            ...
            if ($result) {
                $returnData['code'] = 200;
                $returnData['message'] = 'Image details.';
            } else {
                $returnData['code'] = 400;
                $returnData['message'] = 'No such image data found.';
            }
            $returnData['data'] = $result;
        }
        return json_encode($returnData);
    ...
    public function addImage()
    {
        if (func_num_args() > 0) {
            $data = func_get_arg(0);
            try {
                $result = DB::table($this->table)->insertGetId($data, 'pi_id');
    ...
    public function deleteImagesWhere($where)
    {
        $returnData = array('code' => 400, 'message' => 'Argument Not Passed.', 'data' => null);
        if (func_num_args() > 0) {
            try {
                ...
                if ($result) {
                    $returnData['code'] = 200;
                    $returnData['message'] = 'Image/s deleted.';
                } else {
                    $returnData['code'] = 100;
     ...
     * @author Akash M. Pai <akashpai@globussoft.in>
     */
    public function updateImageWhere()
    {
        $returnData = array('code' => 400, 'message' => 'Argument Not Passed.', 'data' => null);
        if (func_num_args() > 0) {
            $data = func_get_arg(0);
            ...
            try {
                ...
                if ($updatedResult) {
                    $returnData['code'] = 200;
                    $returnData['message'] = 'Image updated successfully.';
                } else {
                    $returnData['code'] = 100;
