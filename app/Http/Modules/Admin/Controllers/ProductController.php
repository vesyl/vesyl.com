...
//            print_a($_FILES);
            ...
            $rules = [
                ...
                'in_stock' => 'required',
                'comment' => 'max:100',
                'shop_id' => 'sometimes|integer|min:0',
            ...
            if ($validator->fails()) {
                ...
                $productData['for_shop_id'] = isset($inputData['shop_id']) ? $inputData['shop_id'] : 0;
                ...
                $resultProductInsert = json_decode($objModelProducts->addNewProduct($productData), true);
                if ($resultProductInsert['data'] > 0) {
                    $insertedProductId = $resultProductInsert['data'];
                    //--------------------------PRODUCT-METADATA----------------------------//
...
//                    $objModelProductFeatureVariantRelation->addFeatureVariantRelation($fvrDataToInsert);
        ...
        return view('Admin/Views/product/addProduct', ['code' => '', 'allCategories' => $allCategories, 'allOptions' => $allOptions, 'featureGroups' => json_decode($allFeatureGroups, true), 'allShops' => $resultAllShops]);
    }
                ...
                case 'deletedProducts':
                    ...
                    return Datatables::of($products)
                        ->addColumn('action', function ($products) {
                            ...
                            $action .= '<li><a href="/admin/edit-product/' . $products['product_id'] . '"><i class="fa fa-pencil" data-id="{{$products->product_id}}"></i>&nbsp;Edit</a></li>';
                            $action .= '</ul>';
                            $action .= '</div>';
                            return $action;
                        })
...
//                    $dataToUpdate['status_set_by'] = $inputData['statussetby'];
                ...
                case "deleteProductImage":
                    ...
                    $productId = $inputData['productId'];
                    $whereForProdImgDel = ['rawQuery' => "for_product_id = ? and pi_id = ?", 'bindParams' => [$productId, $imageId]];
                ...
                case "updateMainImage":
                    $objModelProductImage = ProductImage::getInstance();
                    $dataForResponse = ['code' => '400', 'message' => "Please select an image."];
                    $rules = [
                        'imageId' => 'required'
                    ];
                    $messages = array();
                    $validator = Validator::make($inputData, $rules, $messages);
                    if ($validator->fails()) {
                        $dataForResponse['message'] = $validator->errors()->all();
                    } else {
//                    if ($_FILES) {
                        $imageId = $inputData['imageId'];
                        $productId = $inputData['productId'];

                        $whereForOldImage = ['rawQuery' => 'pi_id = ? and for_product_id = ?', 'bindParams' => [$imageId, $productId]];
                        $reslutOldImage = json_decode($objModelProductImage->getImageWhere($whereForOldImage), true);

                        if ($reslutOldImage['code'] == 200) {
                            $image = "ASd";
                            if (isset($_FILES['product_data']["tmp_name"]["mainimage"])) {
                                $image = $_FILES['product_data']["tmp_name"]["mainimage"];
                            }
                            $mainImageURL = uploadImageToStoragePath($image, 'product_' . $productId, 'product_' . $productId . '_0_' . time() . '.jpg', 724, 1024);

                            if ($mainImageURL) {
                                $oldUrl = $reslutOldImage['data']['image_url'];
                                $dataForProdMainImgUpd = array('image_url' => $mainImageURL);
                                $whereForProdMainImgUpd = ['rawQuery' => 'pi_id = ?', 'bindParams' => [$imageId]];
                                $updatedResult = json_decode($objModelProductImage->updateImageWhere($dataForProdMainImgUpd, $whereForProdMainImgUpd), true);
                                if ($updatedResult['code'] == 200) {
                                    if (!strpos($mainImageURL, 'placeholder') && $reslutOldImage['data']['image_upload_type'] == 0) {
                                        File::delete(storage_path() . $oldUrl);
                                    }
                                }
                                $dataForResponse = $updatedResult;
                            }
                        }
                    }
                    ...
                    switch ($updateFormName) {//to manage requests from a page with multiple forms
                        case "general":
                            ...
                            if ($validator->fails()) {
                                ...
                                $productDataForUpdate['for_shop_id'] = isset($inputData['shop_id']) ? $inputData['shop_id'] : 0;
                                if (array_key_exists('product_type', $inputData))
...
//                                $productDataForUpdate['added_by'] = $userId;
                                ...
                                return redirect()->action(
                                    'Admin\Controllers\ProductController@editProduct', ['productId' => $productId]
                                )->with($returnData);
                            }
                            break;
...
//                                    dd($existingOptionIds);
                                    ...
                                    if (empty($diffNewOVR) && empty($diffDelOVR)) {
                                        ...
                                        if (!empty($diffDelOVR)) {
                                            //delete OVRs
//                                            $clause4DelOVR = implode(',', array_fill(0, count($diffDelOVR) - 1, '?'));//todo why -1 here?
                                            $clause4DelOVR = implode(',', array_fill(0, count($diffDelOVR), '?'));
...
//            dd($dataDefaultImages);
    ...
    private function generateCombinations($array, $currentIndex, $toAppendString, $tempNamesArr)
    {
//        echo "here $currentIndex <br>";
        $arrayLength = count($array);//todo modified js
        $tempArray = array();
        foreach (json_decode($array[$currentIndex]['all_variant_data'], true) as $iVD => $aVD) {
            $temp1 = array();
            $temp1['combinationId'] = $toAppendString . $aVD['VID'];
            $tempExplComb = explode("_", $temp1['combinationId']);
            sort($tempExplComb, SORT_NUMERIC);
            $temp1['combinationIdTmp'] = implode("_", $tempExplComb);
            if (count(explode("_", $temp1['combinationId'])) == $arrayLength) {
//                echo "here new again $currentIndex <br>";
                $temp3['optionName'] = $array[$currentIndex]['option_name'];
                $temp3['optionId'] = $array[$currentIndex]['option_id'];
                $temp3['variantId'] = $aVD['VID'];
                $temp3['variantName'] = $aVD['VN'];
                $temp3['priceModifier'] = $aVD['PM'];
                $temp3['priceModifierType'] = $aVD['PMT'];
                $temp3['weightModifier'] = $aVD['WM'];
                $temp3['weightModifierType'] = $aVD['WMT'];
//                foreach ($tempNamesArr as $iN => $aN) {
//                    $temp1[$iN] = $aN;
//                }
                array_push($temp1, $tempNamesArr, $temp3);
                array_push($tempArray, $temp1);
            }
            if (!empty($array[$currentIndex + 1])) {
                $temp2 = array();
//                echo "here again $currentIndex <br>";
                $tempNamesArr['optionName'] = $array[$currentIndex]['option_name'];
                $tempNamesArr['optionId'] = $array[$currentIndex]['option_id'];
                $tempNamesArr['variantId'] = $aVD['VID'];
                $tempNamesArr['variantName'] = $aVD['VN'];
                $tempNamesArr['priceModifier'] = $aVD['PM'];
                $tempNamesArr['priceModifierType'] = $aVD['PMT'];
                $tempNamesArr['weightModifier'] = $aVD['WM'];
                $tempNamesArr['weightModifierType'] = $aVD['WMT'];
                $temp2 = $this->generateCombinations($array, $currentIndex + 1, $toAppendString . $aVD['VID'] . '_', $tempNamesArr);
                foreach ($temp2 as $iTemp2 => $aTemp2) {
                    array_push($tempArray, $aTemp2);
                }
            }
//            echo "end $currentIndex <br>";
        }
        return $tempArray;
    }
...
//            dd($request->all());
            ...
            if (Input::hasFile('productcsv')) {
                ...
                $fileCSV = Input::file('productcsv')->move($destinationPath, "productcsvsampleadmin.csv");
//                $fileCSV = $destinationPath . "productcsvsampleadmin.csv";
//                echo $fileCSV;
                $results = array();
...
//                            $productMetaData['product_tabs'] = json_encode($inputData['product_tabs']);
                                ...
                                if (!$insertedProductMetaId) {
                                    ...
                                    $allErrors[$keyRow]['message'] = "Product #$keyRow : <b>" . $tempRow['name'] . "</b> was uploaded with some warnings. Please check below warnings and click <a href='/admin/edit-product/$insertedProductId'>here</a> to update.";
...
//                    dd($allErrors);
                    return Redirect::back()
//                    return redirect()->action(
//                        'Admin\Controllers\ProductController@addProductCSV'
//                    )
//                    return view('Admin/Views/product/addProductCSV')
...
//    private function generateCombinations($optionData)
//    {
//
//    }

    public function productDatatablesHandler(Request $request)
    {
        if ($request->isMethod('POST') || $request->isMethod('GET')) {
            ...
            if (isset($requestData['tablename']) && !empty($requestData['tablename'])) {
                ...
                $iTotalRecords = $iDisplayLength = intval($requestData['length']);
                $iDisplayLength = $iDisplayLength < 0 ? $iTotalRecords : $iDisplayLength;
                $iDisplayStart = intval($requestData['start']);
                $sortingOrder = "products.product_id";
                $implodedWhere = '';
...
//                    dd($requestData);
                    $objModelProducts = Products::getInstance();
                    switch ($tableName) {
                        case "productstableadminrewards":
                            $columns = array('products.product_id', 'products.added_date', 'users.username', 'shops.shop_name', 'products.product_name', 'products.price_total', 'products.list_price', 'products.min_qty', 'products.max_qty', 'products.category_id', 'products.available_countries', 'products.products_status');
//                            if (isset($_REQUEST['order'])) {
//                                $sortingOrder = $columns[$_REQUEST['order'][0]['column']];
//                            }

                            $selectedColumns = ['*'];
                            $whereForProducts = ['rawQuery' => 'products.product_type = 0 AND products.product_status IN (1,2)'];
                            $whereForJoin = ['column' => 'product_images.image_type', 'condition' => '=', 'value' => '0'];
                            $resultProducts = json_decode($objModelProducts->getAllProductsWithLimitWhere($whereForProducts, $implodedWhere, $sortingOrder, $iDisplayLength, $iDisplayStart, $whereForJoin, $selectedColumns), true);
                            $allProductsCount = json_decode($objModelProducts->getAllProductsCountWhere($whereForProducts, $implodedWhere, $sortingOrder, $whereForJoin, $selectedColumns), true);
//                            dd($resultProducts);
                            $resultProducts['data'] = array_pad($resultProducts['data'], (-($iDisplayStart + count($resultProducts['data']))), 0);
                            $dataForDT = new Collection();
                            if ($resultProducts['code'] == 200) {
                                foreach ($resultProducts['data'] as $keyResProd => $valueResProd) {
                                    $statusButton = '<button style="margin-left: 10%;" class="btn btn-danger btn-sm" data-id="' . $valueResProd['product_id'] . '" disabled>Inactive</button>';
                                    if ($valueResProd['product_status'] == 1) {
                                        $statusButton = '<button style="margin-left: 10%;" class="btn btn-info btn-sm" data-id="' . $valueResProd['product_id'] . '" disabled>Active</button>';
                                    }
                                    $dataForDT->push([
                                        'product_id' => $valueResProd['product_id'],
                                        'product_name' => '<div class="col-md-3"><img src="' . $valueResProd['image_url'] . '" style="max-height: 70px; width: auto; max-width: 70px;"/></div><div class="col-md-9">' . $valueResProd['product_name'] . "</div>",
                                        'reward_points' => $valueResProd['reward_points'],
                                        'actions' => '<a class="btn btn-default btn-sm edit"><i class="fa fa-pencil"></i></a>',
                                        'status' => $statusButton
                                    ]);
                                }
                                return Datatables::of($dataForDT)
                                    ->with(['recordsTotal' => $allProductsCount['data'], 'recordsFiltered' => $allProductsCount['data'], 'start' => $iDisplayStart])
                                    ->make();
                            }
