...
class OptionController extends Controller
{
    ...
    public function manageOptions()
    {
        ...
        return view('Supplier/Views/option/manageOptions', ['allOptions' => $objProductOptionModel->getAllOptionsWhere(
            ['rawQuery' => 'added_by=? AND status IN (0, 1, 2)', 'bindParams' => [Session::get('fs_supplier')['id']]]
        )]);
    }
    ...
    public function addOption(Request $request)
    {
        ...
        $whereForShops = ['rawQuery' => "user_id = $userId and shop_status in (1,2)"];
        $resultShops = $objModelShop->getAllshopsWhereOld($whereForShops);
        return view('Supplier/Views/option/addOption', ['allShops'=> $resultShops]);
    }
    ...
    public function editOption(Request $request, $id)
    {
        ...
        $whereForOption = ['rawQuery' => 'added_by=? AND option_id =? and status IN (0,1,2)', 'bindParams' => [$userId, $id]];
        ...
        $whereForShops = ['rawQuery' => "user_id = $userId and shop_status in (1,2)"];
        $resultShops = $objModelShop->getAllshopsWhereOld($whereForShops);
        return view('Supplier/Views/option/editOption', ['optionDetails' => $optionDetails, 'allShops' => $resultShops]);
    }
