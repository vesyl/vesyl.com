...
use PDO;
use Input;
use FlashSale\Http\Modules\Supplier\Models\ProductFilterGroup;
use FlashSale\Http\Modules\Supplier\Models\ProductFilterOption;
...
class FilterController extends Controller
{
    ...
    {
        $objProductModel = ProductCategory::getInstance();
        $pendingProducts = $objProductModel->getAllProductsWhereStatus('0');
        return view('Supplier/Views/product/pending-produtcs', ['pendingProducts' => $pendingProducts]);
...
//            dd($allCategories);
            ...
            return view('Supplier/Views/filter/add-new-filtergroup', ['categories' => $allCategories, 'features' => $allFeatures, 'allGroupFilter' => $allGroupFilters]);
        }
        if ($request->isMethod('post')) {
            ...
            if ($validator->fails()) {
                return redirect('/supplier/add-new-filtergroup')
        ...
        return view('Supplier/Views/filter/add-new-filtergroup');

    }

    /**
     * Manage Filter Group Action
     * @param Request $request
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     * @author: Vini Dubey<vinidubey@globussoft.in>
     */
    public function manageFilterGroup(Request $request)
    {
        ...
        if(!empty($getAllFilterGroup)) {
            ...
            return view('Supplier/Views/filter/manage-filtergroup');
        }
        return view('Supplier/Views/filter/manage-filtergroup')->with(['filtergroupdetail' => $getAllFilterGroup,'data'=>$data['added_by']]);
...
//            dd($request);
            ...
            if ($validator->fails()) {
                return redirect('/supplier/edit-filtergroup/' . $id)
                    ->withErrors($validator)
                    ->withInput();
            }
...
//                dd($categoryIds);
        ...
        if ((isset($allCategories)) && (!empty($FilterGroup))) {
            ...
            return view('Supplier/Views/filter/edit-filtergroup', ['editfiltergroup' => $FilterGroup]);
        }
    }
    ...
    public function addFilterVariant(Request $request)
    {
        ...
        return view('Supplier/Views/filter/add-filter-variant');
    }
